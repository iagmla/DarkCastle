...
uint32_t jlyajbe_Q0[4] = {0xed21b71b, 0xe3b4d73a, 0x85f2eb43, 0x9b5240c2};

struct jlyajbe_state {
    uint32_t r[16];
    uint32_t o[8];
    uint32_t y[16];
    int rounds;
...
uint32_t jlyajbe_rotl(uint32_t a, int b) {
    return ((a << b) | (a >> (32 - b)));
}

uint32_t jlyajbe_rotr(uint32_t a, int b) {
    return ((a >> b) | (a << (32 - b)));
}

void jlyajbe_update(struct jlyajbe_state *state) {
    for (int i = 0; i < state->rounds; i++) {
        state->r[8] ^= jlyajbe_rotl((state->r[10] + state->r[3]), 7);
        state->r[15] += jlyajbe_rotl((state->r[14] ^ state->r[6]), 21);
        state->r[9] += jlyajbe_rotl((state->r[11] ^ state->r[0]), 17);
        state->r[5] += jlyajbe_rotl((state->r[7] ^ state->r[12]), 9);
        state->r[2] ^= jlyajbe_rotl((state->r[2] + state->r[9]), 13);
        state->r[11] += jlyajbe_rotl((state->r[9] ^ state->r[2]), 19);
        state->r[4] ^= jlyajbe_rotl((state->r[4] + state->r[15]), 6);
        state->r[6] ^= jlyajbe_rotl((state->r[6] + state->r[13]), 14);
        state->r[13] += jlyajbe_rotl((state->r[12] ^ state->r[4]), 9);
        state->r[0] ^= jlyajbe_rotl((state->r[0] + state->r[11]), 7);
        state->r[7] += jlyajbe_rotl((state->r[5] ^ state->r[14]), 21);
        state->r[14] += jlyajbe_rotl((state->r[13] + state->r[5]), 14);
        state->r[10] += jlyajbe_rotl((state->r[8] + state->r[1]), 13);
        state->r[3] += jlyajbe_rotl((state->r[1] ^ state->r[10]), 19);
        state->r[12] ^= jlyajbe_rotl((state->r[15] + state->r[7]), 6);
        state->r[1] += jlyajbe_rotl((state->r[3] ^ state->r[8]), 17);
    }
...
void jlyajbe_keysetup(struct jlyajbe_state *state, uint8_t *key, uint8_t *nonce) {
    state->rounds = 16;
    ...
    state->r[9] = jlyajbe_Q0[0];
    state->r[11] = jlyajbe_Q0[1];
    state->r[13] = jlyajbe_Q0[2];
    state->r[15] = jlyajbe_Q0[3];

}

void jlyajbe_xor_block(struct jlyajbe_state *state, uint8_t *block) {
...
void jlyajbe_encrypt(char *inputfile, char *outputfile, char *pkfile, char *skfile) {
    struct qloq_ctx ctx;
    ...
    struct jlyajbe_state state;
    int blocklen = 32;
    int bufsize = 32;
    uint8_t nonce[16];
    urandom(nonce, 16);
    jlyajbe_keysetup(&state, key, nonce);
    FILE *infile, *outfile;
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        jlyajbe_update(&state);
        jlyajbe_xor_block(&state, block);
...
void jlyajbe_decrypt(char *inputfile, char *outputfile, char *pkfile, char *skfile) {
    struct qloq_ctx ctx;
    struct qloq_ctx Sctx;
    struct qloq_ctx TMPActx;
    struct qloq_ctx TMPBctx;
