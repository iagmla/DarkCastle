...
void zander3_ofb_encrypt(char *keyfile1, char *keyfile2, char * inputfile, char *outputfile, int key_length, int nonce_length, int mac_length, int kdf_iterations, int password_len,  int keywrap_ivlen, int mask_bytes, int bufsize, unsigned char * passphrase) { 
    ...
    state.S[0] = ((uint64_t)iv[0] << 56) + ((uint64_t)iv[1] << 48) + ((uint64_t)iv[2] << 40) + ((uint64_t)iv[3] << 32) + ((uint64_t)iv[4] << 24) + ((uint64_t)iv[5] << 16) + ((uint64_t)iv[6] << 8) + (uint64_t)iv[7];
    state.S[1] = ((uint64_t)iv[8] << 56) + ((uint64_t)iv[9] << 48) + ((uint64_t)iv[10] << 40) + ((uint64_t)iv[11] << 32) + ((uint64_t)iv[12] << 24) + ((uint64_t)iv[13] << 16) + ((uint64_t)iv[14] << 8) + (uint64_t)iv[15];
    state.S[2] = ((uint64_t)iv[16] << 56) + ((uint64_t)iv[17] << 48) + ((uint64_t)iv[18] << 40) + ((uint64_t)iv[19] << 32) + ((uint64_t)iv[20] << 24) + ((uint64_t)iv[21] << 16) + ((uint64_t)iv[22] << 8) + (uint64_t)iv[23];
    state.S[3] = ((uint64_t)iv[24] << 56) + ((uint64_t)iv[25] << 48) + ((uint64_t)iv[26] << 40) + ((uint64_t)iv[27] << 32) + ((uint64_t)iv[28] << 24) + ((uint64_t)iv[29] << 16) + ((uint64_t)iv[30] << 8) + (uint64_t)iv[31];
    int c = 0;
    int i, b;
    int l = bufsize;
    ...
    for (i = 0; i < blocks; i++) {
        ...
        for (b = 0; b < bblocks; b++) {
            ...
            output[c] = (state.S[0] & 0xFF00000000000000) >> 56;
            output[c+1] = (state.S[0] & 0x00FF000000000000) >> 48;
            output[c+2] = (state.S[0] & 0x0000FF0000000000) >> 40;
            output[c+3] = (state.S[0] & 0x000000FF00000000) >> 32;
            output[c+4] = (state.S[0] & 0x00000000FF000000) >> 24;
            output[c+5] = (state.S[0] & 0x0000000000FF0000) >> 16;
            output[c+6] = (state.S[0] & 0x000000000000FF00) >> 8;
            output[c+7] = (state.S[0] & 0x00000000000000FF);
            output[c+8] = (state.S[1] & 0xFF00000000000000) >> 56;
            output[c+9] = (state.S[1] & 0x00FF000000000000) >> 48;
            output[c+10] = (state.S[1] & 0x0000FF0000000000) >> 40;
            output[c+11] = (state.S[1] & 0x000000FF00000000) >> 32;
            output[c+12] = (state.S[1] & 0x00000000FF000000) >> 24;
            output[c+13] = (state.S[1] & 0x0000000000FF0000) >> 16;
            output[c+14] = (state.S[1] & 0x000000000000FF00) >> 8;
            output[c+15] = (state.S[1] & 0x00000000000000FF);
            output[c+16] = (state.S[2] & 0xFF00000000000000) >> 56;
            output[c+17] = (state.S[2] & 0x00FF000000000000) >> 48;
            output[c+18] = (state.S[2] & 0x0000FF0000000000) >> 40;
            output[c+19] = (state.S[2] & 0x000000FF00000000) >> 32;
            output[c+20] = (state.S[2] & 0x00000000FF000000) >> 24;
            output[c+21] = (state.S[2] & 0x0000000000FF0000) >> 16;
            output[c+22] = (state.S[2] & 0x000000000000FF00) >> 8;
            output[c+23] = (state.S[2] & 0x00000000000000FF);
            output[c+24] = (state.S[3] & 0xFF00000000000000) >> 56;
            output[c+25] = (state.S[3] & 0x00FF000000000000) >> 48;
            output[c+26] = (state.S[3] & 0x0000FF0000000000) >> 40;
            output[c+27] = (state.S[3] & 0x000000FF00000000) >> 32;
            output[c+28] = (state.S[3] & 0x00000000FF000000) >> 24;
            output[c+29] = (state.S[3] & 0x0000000000FF0000) >> 16;
            output[c+30] = (state.S[3] & 0x000000000000FF00) >> 8;
            output[c+31] = (state.S[3] & 0x00000000000000FF);
            c += 32;
        }
        for (b = 0; b < bufsize; b++) {
