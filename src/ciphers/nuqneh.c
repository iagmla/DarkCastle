...
void nuqneh_F(struct nuqneh_state *state) {
    for (int i = 0; i < state->rounds; i++) {
        state->r[2] ^= rotateleft64((state->r[2] + state->r[6]), 19);
        state->r[1] += rotateleft64((state->r[3] ^ state->r[4]), 13);
        state->r[6] ^= rotateleft64((state->r[6] ^ state->r[2]), 18);
        state->r[4] ^= rotateleft64((state->r[4] ^ state->r[3]), 7);
        state->r[7] += rotateleft64((state->r[5] + state->r[1]), 31);
        state->r[0] ^= rotateleft64(state->r[0] + state->r[7], 5);
        state->r[5] += rotateleft64((state->r[7] + state->r[0]), 12);
        state->r[3] += rotateleft64((state->r[1] + state->r[5]), 26);
        state->r[0] ^= rotateleft64((state->r[2] ^ state->r[7]), 5);
        state->r[6] += rotateleft64((state->r[0] ^ state->r[4]), 18);
        state->r[3] ^= rotateleft64((state->r[3] + state->r[5]), 26);
        state->r[5] ^= rotateleft64((state->r[5] + state->r[2]), 12);
        state->r[7] ^= rotateleft64((state->r[7] + state->r[6]), 31);
        state->r[1] += rotateleft64((state->r[1] + state->r[0]), 13);
        state->r[2] += rotateleft64((state->r[4] ^ state->r[1]), 19);
        state->r[4] += rotateleft64((state->r[6] ^ state->r[3]), 7);
    }
...
void nuqneh_keysetup(struct nuqneh_state *state, uint8_t *key, uint8_t *nonce) {
...
void nuqneh_xor_block(struct nuqneh_state *state, uint8_t *block) {
...
void nuqneh_encrypt(char *inputfile, char *outputfile, char *pkfile, char *skfile) {
    struct qloq_ctx ctx;
    ...
    struct nuqneh_state state;
    int blocklen = 32;
    int bufsize = 32;
    uint8_t nonce[16];
    urandom(nonce, 16);
    nuqneh_keysetup(&state, key, nonce);
    FILE *infile, *outfile;
...
void nuqneh_decrypt(char *inputfile, char *outputfile, char *pkfile, char *skfile) {
    struct qloq_ctx ctx;
    struct qloq_ctx Sctx;
    struct qloq_ctx TMPActx;
    struct qloq_ctx TMPBctx;
